...
import { canAccess, createMockAuthGuards, getRolePermissions, hasPermission, PERMISSIONS, ROLE_HIERARCHY, ROLE_PERMISSIONS, } from "./guards";
describe("Auth Guards", () => {
    ...
    describe("Mock Guards", () => {
        it("should create functional mock guards", async () => {
            ...
            const membership = await mockGuards.assertMembership({}, "user123", "workspace123");
            expect(membership.userId).toBe("user123");
            expect(membership.role).toBe("editor");
            const hasPermission = await mockGuards.checkPermission({}, "user123", "workspace123", "test:permission");
