...
describe("Workspace Functions - convex-test", () => {
	...
	describe("inviteMember", () => {
		test.skip("requires authentication - SKIPPED: Auth integration", async () => {
			/**
			 * LIMITATION DOCUMENTED:
			 * inviteMember depends on auth.getUserId() and complex membership checks
			 * that don't integrate well with convex-test's identity system.
			 */
			const t = convexTest(schema, modules);
			// Test validation using tokenIdentifier pattern

			const invitePromise = t
				.withIdentity({
					tokenIdentifier: "testing|invite_user",
			...
			await expect(invitePromise).rejects.toThrow("Authentication required");
		});
