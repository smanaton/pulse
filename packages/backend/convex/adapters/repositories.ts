...
export class ConvexIdeaRepository extends BaseConvexRepository implements IIdeaRepository {
	async create(data: IdeaCreateInput & { createdBy: Id<"users"> }): Promise<Id<"ideas">> {
	...
	async findByWorkspace(workspaceId: Id<"workspaces">, limit = 50): Promise<any[]> {
	...
	async search(options: IdeaSearchOptions): Promise<any[]> {
		let query = this.ctx.db
			.query("ideas")
			.withIndex("by_workspace", (q) => q.eq("workspaceId", options.workspaceId))
			.filter((q) => q.eq(q.field("deletedAt"), undefined));
		
		// Apply additional filters
		if (options.projectId) {
			query = query.filter((q) => q.eq(q.field("projectId"), options.projectId));
		}
...
export class ConvexFolderRepository extends BaseConvexRepository implements IFolderRepository {
	async create(data: { 
	...
	async delete(id: Id<"folders">): Promise<void> {
		...
		if (ideasInFolder) {
			throw new Error("Cannot delete folder containing ideas. Move or delete ideas first.");
		}
...
export function createRepositories(ctx: MutationCtx | QueryCtx): IRepositoryContainer {
	return {
