...
import { AIError, type AIContentService } from "./types";
...
export class AIContentServiceImpl implements AIContentService {
   ...
   */
  private prioritizeTags(suggestedTags: string[], existingTags: string[]): string[] {
    ...
    const newSuggestions = suggestedTags.filter(tag => 
      !lowerExistingTags.includes(tag)
    );
