...
describe("API Keys System", () => {
  ...
  afterEach(async () => {
    // Clean up test data
    await t.run(async (ctx) => {
      const tables = ["apiKeys", "workspaceMembers", "workspaces", "users", "events"];
  ...
  describe("API Key Generation", () => {
    ...
    test("Given_NonAdminUser_When_GeneratesKey_Then_ThrowsPermissionError", async () => {
      // Act & Assert
      await expect(
        t.withIdentity({ tokenIdentifier: "test|member123" })
          .mutation(api.apiKeys.generate, {
            workspaceId,
      ...
      await t.run(async (ctx) => {
        const membership = await ctx.db
          ...
          .withIndex("by_workspace_user", (q) => 
            q.eq("workspaceId", workspaceId).eq("userId", memberUserId))
          .unique();
        if (membership) {
