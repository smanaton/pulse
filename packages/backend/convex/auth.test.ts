...
describe("Authentication System", () => {
	...
	describe("Authentication Security", () => {
		...
		test("Given_MultipleUsers_When_Authenticated_Then_IsolatesUserData", async () => {
			...
			const user1Data = await t
				.withIdentity(user1Identity)
				.query(api.users.getCurrentUser, {});
			const user2Data = await t
				.withIdentity(user2Identity)
				.query(api.users.getCurrentUser, {});

			expect(user1Data?.name).toBe("User 1");
			expect(user2Data?.name).toBe("User 2");
			expect(idOf(user1Data, "user1Data")).not.toBe(
				idOf(user2Data, "user2Data"),
			);
		});
	});
	...
	describe("Session Management", () => {
		test("Given_AuthenticatedUser_When_RepeatedRequests_Then_MaintainsSession", async () => {
			...
			const request1 = await t
				.withIdentity(identity)
				.query(api.users.getCurrentUser, {});
			const request2 = await t
				.withIdentity(identity)
				.query(api.users.getCurrentUser, {});
			const request3 = await t
				.withIdentity(identity)
				.query(api.users.getCurrentUser, {});

			// Assert - All requests return consistent data
			expect(idOf(request1, "request1")).toBe(idOf(request2, "request2"));
			expect(idOf(request2, "request2")).toBe(idOf(request3, "request3"));
