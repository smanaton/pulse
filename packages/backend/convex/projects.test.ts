...
describe("Projects System", () => {
	...
	describe("Project Creation", () => {
		...
		test("Given_AdminUser_When_CreatesProjectWithOwner_Then_SetsSpecifiedOwner", async () => {
			...
			const members = await t.run(async (ctx) => {
				return await ctx.db
					...
					.filter((q) => q.eq(q.field("projectId"), result))
					.collect();
			});
		...
		test("Given_MultipleProjects_When_Created_Then_AssignsSortKeysCorrectly", async () => {
			...
			const projects = await t.run(async (ctx) => {
				return await Promise.all([ctx.db.get(project1), ctx.db.get(project2)]);
			});
	...
	describe("Project Listing", () => {
		let activeProjectId: Id<"projects">;
		let completedProjectId: Id<"projects">;
		let onHoldProjectId: Id<"projects">;
	...
	describe("Project Reordering", () => {
		let project1Id: Id<"projects">;
		let project2Id: Id<"projects">;
		let project3Id: Id<"projects">;
		...
		test("Given_ProjectMember_When_UpdatesRole_Then_UpdatesSuccessfully", async () => {
			...
			const member = await t.run(async (ctx) => {
				return await ctx.db
					.query("projectMembers")
					.filter((q) => q.eq(q.field("projectId"), projectId))
					.filter((q) => q.eq(q.field("userId"), editorUserId))
					.first();
			});
