...
test.describe("@smoke Critical User Journeys", () => {
	...
	test("Sign in and create idea journey", async ({ page }) => {
		...
		await page
			.getByLabel("Content")
			.fill("This is a test idea created by E2E automation");
	...
	test("Web clipper integration flow", async ({ page, context: _context }) => {
		...
		await expect(page.getByText("Clipped to Pulse")).toBeVisible({
			timeout: 5000,
		});
	});
		...
		const _metrics = await page.evaluate(() => {
			return new Promise((resolve) => {
				new PerformanceObserver((list) => {
					...
					) as PerformanceEventTiming | undefined;
