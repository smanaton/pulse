...
function backupExistingSchema() {
	if (existsSync(SCHEMA_OUTPUT)) {
		try {
			...
			console.log("✅ Backed up existing schema to schema.backup.ts");
		} catch (error) {
			console.warn("⚠️ Could not backup existing schema:", error.message);
		}
	}
}
...
function main() {
	...
	try {
		...
		console.log("✅ Successfully merged schemas");
